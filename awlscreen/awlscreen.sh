...
rm -f "$SOCKET_FILE"
xwinwrap -fdt -ni -b -g $RESOLUTION -- mpv -wid WID --loop --no-audio --panscan=1.0 --input-ipc-server="$SOCKET_FILE" "$VIDEO_PATH" &
...
while true; do
    ...
    if [ "$SHOULD_STOP" = true ] && [ "$STATUS_VAR" -eq 0 ]; then     
        STATUS_VAR=1
        # FIXED: Pause instead of killing (Frees hardware resources instantly)
        echo "set pause yes" | socat - "$SOCKET_FILE" >/dev/null 2>&1
    elif [ "$SHOULD_STOP" = false ] && [ "$STATUS_VAR" -eq 1 ]; then 
        STATUS_VAR=0
        # FIXED: Unpause instantly with 0ms black screen delay
