...
EXPECTED_DIR="$USER_HOME/a-wallpaper-lite/wayland"

# ts is to make it so you dont screw it up
if [ "$FOLDERIG" != "$EXPECTED_DIR" ]; then
    echo ""
    echo "ERROR: a-wallpaper-lite folder is not in your home directory"
    echo "the current folder is:  $FOLDERIG"
    echo "a-wallpaper-lite folder needs to be in: $EXPECTED_DIR"
    echo ""
    echo "Please move the 'a-wallpaper-lite' folder to $USER_HOME/ and try again."
    exit 1
fi
...
cd "$FOLDERIG"
chmod +x awlscreenwayland.sh
chmod +x awlscreenstart.sh
...
install_deps() {
    ...
    if command -v apt-get &> /dev/null; then
        ...
        apt-get update
        apt-get install -y mpv zenity git socat gcc ffmpeg meson ninja
    elif command -v pacman &> /dev/null; then
        echo "Detected Arch-based system (pacman)..."
        pacman -Sy --noconfirm mpv zenity git socat gcc ffmpeg meson ninja
    elif command -v dnf &> /dev/null; then
        echo "Detected Fedora/RHEL-based system (dnf)..."
        dnf install -y mpv zenity git socat gcc ffmpeg meson ninja
    elif command -v zypper &> /dev/null; then
        echo "Detected openSUSE-based system (zypper)..."
        zypper install -y mpv zenity git socat gcc ffmpeg meson ninja
    else
	...
	echo "4) opensuse (zypper)"
        ...
        case $choice in
            1) apt-get update && apt-get install -y mpv zenity git socat gcc ffmpeg meson ninja ;;
            2) pacman -Sy --noconfirm mpv zenity git socat gcc ffmpeg meson ninja ;;
            3) dnf install -y mpv zenity git socat gcc ffmpeg meson ninja ;;
	    4) zypper install -y mpv zenity git socat gcc ffmpeg meson ninja ;;
	...
	rm -rf mpvpaper
        cd "$FOLDERIG" || exit
        echo "mpvpaper successfully compiled and installed!"
    else
...
echo "SOME REPOS DONT HAVE MPVPAPER 	YOU WILL NEED TO INSTALL THIS WITH GIT AND MAKE IF THE AUTO INSTALLER DIDNT DO IT FOR YOU"
